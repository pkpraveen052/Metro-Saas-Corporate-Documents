...
class CorporateDocumentAttachment(models.Model):
    ...
    def create(self, vals):
        obj = super(CorporateDocumentAttachment, self).create(vals)
        if not vals.get('report_id', False):
            report_obj = self.env.ref('metro_corporate_docs.ir_actions_report_corporate_address_change')
            if report_obj:
            ...
            report_id = self.env['ir.actions.report'].sudo().create({
                'name': 'Address Change - ' + str(obj.name),
                'report_type': 'docx-docx',
                'model': 'corporate.document.attachment',
                'print_report_name': "'Address_Change - %s' % (object.year)",
    ...
    def write(self, vals):
        res = super(CorporateDocumentAttachment, self).write(vals)
        for obj in self:
            if vals.get('name'):
    ...
    def unlink(self):
        ...
        return super(CorporateDocumentAttachment, self).unlink()
