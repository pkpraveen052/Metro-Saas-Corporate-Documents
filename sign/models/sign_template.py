...
class SignItemParty(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        """Override search to apply domain filter for Invoice User group."""
        ctx = self._context
        print("\nSignItemParty Search() ctx ========================", ctx)
        print("args ===",args)
        return super(SignItemParty, self).search(args, offset=offset, limit=limit, order=order, count=count)
